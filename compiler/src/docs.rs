...
use types::{
    ClassId, ClassKind, Database, Location, MethodId, MethodKind, ModuleId,
    TraitId, TypeBounds,
};
...
fn method_kind(kind: MethodKind) -> i64 {
    match kind {
        MethodKind::Instance => 0,
        MethodKind::Mutable => 1,
        MethodKind::Moving => 2,
        MethodKind::Async => 3,
        MethodKind::AsyncMutable => 4,
        MethodKind::Destructor => 5,
        MethodKind::Static | MethodKind::Constructor => 6,
        MethodKind::Extern => 7,
    }
}
...
impl<'a> GenerateDocumentation<'a> {
    pub(crate) fn run_all(
        ...
        generate_metadata(state, &directories, private)?;
        Ok(())
    }
    ...
    fn methods(&self, methods: Vec<MethodId>) -> Json {
        ...
        for id in methods {
            ...
            obj.add("name", Json::String(name));
            obj.add("kind", Json::Int(method_kind(kind)));
...
fn generate_metadata(
    ...
    meta.add("target", Json::String(state.config.target.to_string()));
    meta.add("private", Json::Bool(private));
